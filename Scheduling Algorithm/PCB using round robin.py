    ...
    def printPCB(self, myprocess, QT):
        if not myprocess.isexecuted:
            ...
            if (self.processlist[self.PC].resume <=0):
                ...
                table.add_row(["Value of PC", f"process {self.PC + 1} [ instruction {self.processlist[self.PC].resume} ]"])
            
            if myprocess.resource:
                table.add_row(["Resource required", f"process {myprocess.processid} [ instruction {self.IR} ]"])
            else:
                table.add_row(["Value of IR", f"process {myprocess.processid} [ instruction {self.IR} ]"])
                executed_str = ", ".join([str(i + 1) for i in range(myprocess.executed)])
                remaining_str = ", ".join([str(i + 1) for i in range(myprocess.executed, myprocess.executiontime)])
                table.add_row(["Executed instructions", f"[ instruction {executed_str} ]"])
                table.add_row(["Remaining instructions", f"[ {remaining_str} ]"])

            if myprocess.resume >= 0:    
                table.add_row(["Will resume from", f"process {myprocess.processid} [ instruction {myprocess.resume} ]"])
            else:
                table.add_row(["Will resume from", f"Process {myprocess.processid} executed completely"])
            table.add_row(["State", "Running" if myprocess.state == 'R' else "Blocked"])
